...
if (window.location.href.indexOf('map') < 0) {
    ...
    function finish(list) {
        ...
        <tr style="background-color:${backgroundColor}">
        <td style="text-align:center; width:auto; background-color:${backgroundColor}"><input type="button"  class="btn evt-confirm-btn btn-confirm-yes" id="findCoords" onclick="findBarbs()" value="Find barbs" ><input type="button" style="display:none;" class="btn evt-confirm-btn btn-confirm-yes" id="compareCoordsBtn" onclick="grabLAPages()" value="Remove LA barbs">
        </td></tr>
          </tr>
    ...
    function findBarbs() {
        ...
        for (var j = 0; j < inRangeBarb.length; j++) {
            ...
            arrayCoords+= '"'+ inRangeBarb[j].targetX + "|" + inRangeBarb[j].targetY+'",';
        }
        arrayCoords=arrayCoords.substring(0, arrayCoords.length - 1) +"]";
        barbCoordArray=JSON.parse(arrayCoords);
        var distance = [];
        ...
        for (k = 0; k < inRangeBarb.length; k++) {
            ...
            distance.push({"coord":target, "distance":calculateDistance(target, coordinateHome)});
        }
        distance.sort((a,b) => a.distance - b.distance);
        console.log(distance);
        let sortedBarbs="";
        arrayCoords="[";
        for(let l=0;l<distance.length;l++)
        {
            arrayCoords+='"'+distance[l].coord+'",';
            sortedBarbs+=distance[l].coord+ " ";
        }
        arrayCoords=arrayCoords.substring(0, arrayCoords.length - 1) +"]";
        barbCoordArray=JSON.parse(arrayCoords);
        $("#coords")[0].innerHTML = `<font color="${titleColor}"><p>Total found: ${barbCoordArray.length}</p><p>${sortedBarbs}</p></font>`;
    ...
    function grabLAPages() {
        //grab all pages from LA
        $.get('game.php?&screen=am_farm', function (response) {
            // check how many extra pages
            laPages = $(response).find("#plunder_list_nav").eq(0).find(".paged-nav-item");
            if (laPages.length > 0) {
                // found multiple pages
                for (var i = 0; i < laPages.length; i++) {
                    //adding all pages
                    laPagesLinks.push('/game.php?&screen=am_farm&order=distance&dir=asc&Farm_page=' + i);
                }
            }
            else {
                laPagesLinks = "/game.php?&screen=am_farm";
            }
        }).done(
            function () {
                console.log("done");
                $.getAll(laPagesLinks,
                    (i, data) => {
                        ...
                        let currentPageList = $(data).find("#plunder_list")[0].innerHTML.match(/\d+\|\d+/ig);
                        coordinates = coordinates.concat(currentPageList);
                    },
                    () => {
                        console.log(coordinates);
                        console.log("Difference: ");
                        let result = compareArray(barbCoordArray, coordinates);
                        console.log(result);
                        let displayResult="";
                        ...
                        //display found coordinates here
                        $("#coords")[0].innerHTML = `<font color="${titleColor}"><p>Total left without LA: ${result.length}</p><p>${displayResult}</p></font>`;
    ...
    function compareArray(array1, array2) {
        array1 = Object.values(array1).filter(val => !Object.values(array2).includes(val));
        return array1;
    }
